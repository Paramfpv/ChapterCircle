...
// Add your YouTube links here — replace VIDEO_ID_X with actual IDs or full URLs
const youtubeLinks = {
    1: "https://youtu.be/AFe9mq2yI9g?si=hSIUt5Pioed9s3Am",
    2: "https://youtu.be/7UsG86vV7yc?si=HTv2ceHqYTAXEco-",
    3: "https://youtu.be/j2NQI9qG_kg?si=5RFASj1X0qVHn-mY",
    4: "https://youtu.be/0l-LnM9yFEc?si=hPYECp08A4wZ8Cdt",
    5: "https://youtu.be/fVFIe8gfLuQ?si=gjd4T2uk5KqX_coU",
    6: "https://youtu.be/kzmxH5IYFVg?si=b31v8vUJMmHHGegM",
};
...
// --- MODIFIED BOOK CARD CLICK HANDLERS ---
// Now opens YouTube links in a new tab instead of modal
bookCards.forEach(card => {
    card.addEventListener('click', () => {
        const bookId = card.getAttribute('data-book');
        const youtubeURL = youtubeLinks[bookId];
        if (youtubeURL) {
            window.open(youtubeURL, '_blank');
        } else {
            console.warn('No YouTube URL found for book id:', bookId);
        }
    });
});

// Open video modal (no longer used in option 1, but kept if you want to use elsewhere)
function openVideoModal(book) {
    modalTitle.textContent = book.title;
    videoTitle.textContent = book.title;
    videoAuthor.textContent = book.author;
